...
// How each person took part. Records saved before the online form existed have no
// "mode" field, and they were all face-to-face registrations.
const MODES = { face: 'Face-to-face', online: 'Online' };
const modeOf = (r) => (r && (r.mode === 'online' || r.mode === 'absent') ? 'online' : 'face');
...
const DEFAULT_EVENT = {
  ...
  onlineOpen: true,
  onlineClosedMessage: 'Registration is now closed. Thank you for your interest!'
};
...
function validateEvent(body) {
  ...
  str('onlineClosedMessage', 200, true);
  ...
  d.onlineOpen = body.onlineOpen === true;
...
const ONLINE_TEMPLATE = fs.readFileSync(path.join(PUBLIC_DIR, 'online.html'), 'utf8');
...
function renderPage(template, isOpen, closedMessage) {
  ...
  const vars = {
    ...
    closedMessage: esc(closedMessage),
  ...
  html = keep(html, 'open', isOpen);
  html = keep(html, 'closed', !isOpen);
  html = keep(html, 'tagline', !!ev.tagline);
  html = keep(html, 'contact', !!ev.contactEmail);
  return html.replace(/\{\{(\w+)\}\}/g, (m, k) => (k in vars ? vars[k] : ''));
}

// Face-to-face registration page (the main link)
const renderIndex = () => renderPage(INDEX_TEMPLATE, eventSettings.registrationOpen, eventSettings.closedMessage);
// Online participation page (the second link, no certificate)
const renderOnline = () => renderPage(ONLINE_TEMPLATE, eventSettings.onlineOpen, eventSettings.onlineClosedMessage);
...
function validate(body, withMode) {
  const errors = {};
  const data = {
  ...
  data.mode = withMode ? 'online' : 'face';
  return { errors, data };
}
...
// Generous on purpose: many students on the same campus Wi-Fi can share one address.
const registerLimited = makeLimiter(40, 60 * 1000);
...
async function handleAdminApi(req, res, pathname, url) {
  ...
  if (pathname === '/api/admin/registrations') {
    if (req.method === 'GET') {
      return sendJson(res, 200, { ok: true, registrations: registrations.slice().reverse().map((r) => Object.assign({}, r, { mode: modeOf(r) })) });
    }
...
function handleCsv(req, res) {
  ...
  const head = ['Reference', 'Submitted (Manila time)', 'Type', 'Full name', 'Course/Program', 'Year level', 'Section', 'Contact number', 'Email'];
  const rows = registrations.map((r) => [
    csvCell(r.ref),
    csvCell(new Date(r.createdAt).toLocaleString('en-PH', { timeZone: 'Asia/Manila' })),
    csvCell(MODES[modeOf(r)]),
    csvCell(r.fullName),
    csvCell(r.course),
    csvCell(r.yearLevel),
    csvCell(r.section),
...
async function handleRegister(req, res) {
  ...
  if (body.website) {
    return sendJson(res, 200, { ok: true, registration: { ref: 'P77-0000', fullName: clean(body.fullName), createdAt: new Date().toISOString() } });
  }
  ...
  if (existing) {
    ...
    if (existing.fullName.toLowerCase() === data.fullName.toLowerCase()) {
      ...
      return sendJson(res, 200, {
        ok: true,
        existing: true,
        registration: { ref: existing.ref, fullName: existing.fullName, createdAt: existing.createdAt }
      });
    }
    return sendJson(res, 409, {
      ok: false,
  ...
  return sendJson(res, 201, {
    ok: true,
    registration: { ref: record.ref, fullName: record.fullName, createdAt: record.createdAt }
  });
}

/* ------------------------------------------------- online participation API */

// The second link. Same form as the main page, but no certificate is given.
async function handleParticipate(req, res) {
  if (registerLimited(clientIp(req))) {
    return sendJson(res, 429, { ok: false, message: 'Too many attempts. Please wait a minute and try again.' });
  }
  if (!eventSettings.onlineOpen) {
    return sendJson(res, 403, { ok: false, message: eventSettings.onlineClosedMessage });
  }
  ...
  // Honeypot: real people never fill this hidden field. Pretend success.
  if (body.website) {
    return sendJson(res, 200, { ok: true, registration: { ref: 'P77-0000', fullName: clean(body.fullName), createdAt: new Date().toISOString(), mode: 'online' } });
  }

  const { errors, data } = validate(body, true);
  if (Object.keys(errors).length) {
    return sendJson(res, 422, { ok: false, message: 'Please check the highlighted fields.', errors });
  }

  const summary = (r) => ({ ref: r.ref, fullName: r.fullName, createdAt: r.createdAt, mode: modeOf(r) });
  ...
  if (existing) {
    ...
    // Same person again: give back the original record.
    return sendJson(res, 200, { ok: true, existing: true, registration: summary(existing) });
  }
  ...
  try {
    ...
    return sendJson(res, 500, { ok: false, message: 'We could not save your response. Please try again.' });
  }

  return sendJson(res, 201, { ok: true, registration: summary(record) });
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (pathname === '/api/participate') {
      if (req.method !== 'POST') return sendJson(res, 405, { ok: false, message: 'Method not allowed' });
      return await handleParticipate(req, res);
    }
    ...
    if (pathname === '/online' || pathname === '/online/' || pathname === '/online.html') {
      return send(res, 200, req.method === 'HEAD' ? '' : renderOnline(), {
        'Content-Type': 'text/html; charset=utf-8',
        'Cache-Control': 'no-store'
      });
    }
