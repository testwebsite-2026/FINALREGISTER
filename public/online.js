(function () {
  ...
  var EVENT = {};
  try { EVENT = JSON.parse($('event-data').textContent) || {}; } catch (e) { /* fine */ }
  ...
  var ORDER = ['fullName', 'course', 'yearLevel', 'section', 'contact', 'email'];
  ...
  function showDone(reg) {
    var name = String(reg.fullName || '').trim();
    var first = name.split(' ')[0] || name;
    $('done-message').textContent = 'Thank you, ' + first + '! Your registration is confirmed.';
  ...
  if (EVENT.onlineOpen === false) $('btn-again').hidden = true;
